...
export class MobileAPI {
  ...
  // Authentication
  async authenticateUser(email: string, password: string): Promise<MobileUser | null> {
    try {
  ...
  // Orders
  async createOrder(userId: string, items: OrderItem[]): Promise<MobileOrder | null> {
    try {
  ...
  // Bookings
  async createBooking(userId: string, date: string, timeSlot: string, guestCount: number): Promise<MobileBooking | null> {
    try {
      const { data, error } = await supabase
        .from('bookings')
        .insert({
  ...
  async completeNFCExperience(userId: string, experienceId: string): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('user_nfc_completions')
        .insert({
          user_id: userId,
  ...
  // Push notifications
  async registerForPushNotifications(userId: string, deviceToken: string): Promise<boolean> {
    try {
      const { error } = await supabase
