...
export function formatIDR(n: number, opts: { showCode?: boolean } = {}) {
  const s = new Intl.NumberFormat('id-ID', { maximumFractionDigits: 0 }).format(n);
...
function toWordsID_uptoHundreds(n: number): string {
  ...
  if (n < 100) {
    ...
    return toWordsID_uptoHundreds(d) + ' puluh' + (r ? ' ' + toWordsID_uptoHundreds(r) : '');
  }
  if (n < 200) return 'seratus' + (n % 100 ? ' ' + toWordsID_uptoHundreds(n % 100) : '');
  const h = Math.floor(n / 100), r = n % 100;
  return toWordsID_uptoHundreds(h) + ' ratus' + (r ? ' ' + toWordsID_uptoHundreds(r) : '');
}
...
const RU_1_19 = ['ноль','один','два','три','четыре','пять','шесть','семь','восемь','девять','десять','одиннадцать','двенадцать','тринадцать','четырнадцать','пятнадцать','шестнадцать','семнадцать','восемнадцать','девятнадцать'];
...
function toWordsRU(n: number): string {
  if (n === 0) return 'ноль';
  let out: string[] = [];
  const mln = Math.floor(n/1_000_000); n = n % 1_000_000;
...
export function idrWithWords(n: number, locale: 'id'|'ru'='id', withCode = true) {
