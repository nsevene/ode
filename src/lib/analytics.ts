...
export class PerformanceMonitor {
  ...
  monitorCoreWebVitals() {
    ...
    new PerformanceObserver((list) => {
      ...
      AnalyticsManager.getInstance().trackPerformance('LCP', lastEntry.startTime);
    ...
    new PerformanceObserver((list) => {
      ...
      entries.forEach((entry) => {
        AnalyticsManager.getInstance().trackPerformance('FID', entry.processingStart - entry.startTime);
      });
  ...
  monitorPageLoad() {
    ...
    window.addEventListener('load', () => {
      ...
      AnalyticsManager.getInstance().trackPerformance('page_load_time', navigation.loadEventEnd - navigation.fetchStart);
      AnalyticsManager.getInstance().trackPerformance('dom_content_loaded', navigation.domContentLoadedEventEnd - navigation.fetchStart);
    });
  }
}
...
export class BusinessMetrics {
  ...
  trackTenantApplication(applicationData: any) {
    AnalyticsManager.getInstance().trackBusinessMetric('tenant_application', 1, {
  ...
  trackInvestorConsultation(consultationData: any) {
    AnalyticsManager.getInstance().trackBusinessMetric('investor_consultation', 1, {
