        ...
        try {
          if (link.type === 'internal') {
            ...
            const internalRoutes = [
              '/', '/taste-compass', '/quick-booking', '/wine-staircase',
              '/virtual-tour', '/events', '/my-bookings', '/auth', '/dashboard',
              '/food-ordering', '/vendors', '/blog', '/chefs-table', '/lounge'
            ];
            const status: 'working' | 'broken' = internalRoutes.includes(link.path) ? 'working' : 'broken';
            return { ...link, status };
          } else {
                  ...
                  {brokenLinks.map((link, index) => (
                    <div key={index} className="p-3 bg-red-50 rounded-lg flex justify-between items-center">
                      <div>
                        ...
                        <span className="text-sm text-gray-600 ml-2">({link.path})</span>
                      </div>
...
export const allAppLinks = [
  ...
  { path: '/wine-staircase', label: 'Wine Staircase', type: 'internal' as const },
  { path: '/virtual-tour', label: 'Virtual Tour', type: 'internal' as const },
  { path: '/events', label: 'Events', type: 'internal' as const },
  { path: '/my-bookings', label: 'Мои бронирования', type: 'internal' as const },
  ...
  { path: '/virtual-tour-fallback', label: 'Fallback тур', type: 'internal' as const },
  
  // External links
  { path: 'https://wa.me/79251234567', label: 'WhatsApp', type: 'external' as const },
  { path: 'https://instagram.com/odefoodhall', label: 'Instagram', type: 'external' as const },
  { path: 'mailto:selena@odefoodhall.com', label: 'Email главный', type: 'external' as const },
  { path: 'mailto:selena@odefoodhall.com', label: 'Email вендоров', type: 'external' as const },
