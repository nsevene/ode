...
const PerformanceDashboard = () => {
  ...
  const getMetricStatus = (value: number, threshold: number, lowerIsBetter: boolean = true) => {
    const isGood = lowerIsBetter ? value <= threshold : value >= threshold;
    return isGood ? 'good' : 'warning';
  };

  const formatMetric = (value: number, unit: string = 'ms', decimals: number = 0) => {
    if (unit === 'MB') {
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab} className="w-full">
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-center">
                  ...
                  <Badge className={`mt-2 ${getStatusColor(getMetricStatus(metrics.loadTime, thresholds.loadTime))}`}>
                    {getMetricStatus(metrics.loadTime, thresholds.loadTime) === 'good' ? 'Good' : 'Warning'}
                  </Badge>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-center">
                  ...
                  <Badge className={`mt-2 ${getStatusColor(getMetricStatus(metrics.memoryUsage, thresholds.memoryUsage))}`}>
                    {getMetricStatus(metrics.memoryUsage, thresholds.memoryUsage) === 'good' ? 'Good' : 'Warning'}
                  </Badge>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="pt-6">
                <div className="text-center">
                  ...
                  <Badge className="mt-2 bg-blue-100 text-blue-800">
                    {metrics.networkErrors > 0 ? `${metrics.networkErrors} Errors` : 'No Errors'}
                  </Badge>
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="core-vitals" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { name: 'LCP', value: metrics.lcp, threshold: thresholds.lcp, unit: 'ms', description: 'Largest Contentful Paint' },
              { name: 'FID', value: metrics.fid, threshold: thresholds.fid, unit: 'ms', description: 'First Input Delay' },
              { name: 'CLS', value: metrics.cls, threshold: thresholds.cls, unit: '', description: 'Cumulative Layout Shift' },
              { name: 'FCP', value: metrics.fcp, threshold: thresholds.fcp, unit: 'ms', description: 'First Contentful Paint' },
              { name: 'TTFB', value: metrics.ttfb, threshold: thresholds.ttfb, unit: 'ms', description: 'Time to First Byte' }
            ].map((metric, index) => {
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between text-sm text-muted-foreground mb-1">
                        <span>0</span>
                        <span>Threshold: {formatMetric(metric.threshold, metric.unit)}</span>
                      </div>
                      <Progress 
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatMetric(metrics.firstPaint)}</span>
                </div>
                <div className="flex justify-between">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatMetric(metrics.memoryUsage, 'MB', 1)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Memory Limit:</span>
                  <span className="font-medium">{formatMetric(metrics.memoryLimit, 'MB', 1)}</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="space-y-3">
                {recommendations.length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">
                      Great! Your performance metrics are within optimal thresholds.
                    </p>
                  </div>
                )}
              </div>
