...
import { Activity, Zap, Clock, Database, Wifi, WifiOff, AlertTriangle } from 'lucide-react';
...
export const OptimizedPerformanceMonitor: React.FC<OptimizedPerformanceMonitorProps> = ({
  ...
  const updateMetrics = useCallback(() => {
    try {
      ...
      let memoryUsage = 0;
      if (performanceInfo.memory && performanceInfo.memory.totalJSHeapSize > 0) {
        memoryUsage = Math.min(100, Math.round(
          (performanceInfo.memory.usedJSHeapSize / performanceInfo.memory.totalJSHeapSize) * 100
        ));
      }
      ...
      if (performance.timeOrigin) {
        renderTime = Math.max(0, Math.round(performance.now() - performance.timeOrigin));
      }
    ...
    const measureFPS = () => {
      try {
        ...
        if (currentTime - lastTime >= 1000) {
          ...
          setMetrics(prev => ({ ...prev, fps: Math.min(60, Math.max(0, fps)) }));
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span className={metrics.networkStatus === 'online' ? 'text-green-400' : 'text-red-400'}>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className={getCacheStatusColor(metrics.cacheStatus)}>
                  {metrics.cacheStatus === 'enabled' ? 'E' : metrics.cacheStatus === 'disabled' ? 'D' : '?'}
                </span>
              </div>
            </motion.div>
          )}
