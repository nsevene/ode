...
const DEFAULT_IMAGES: GalleryImage[] = [
  ...
  },
  {
    ...
    description: "Аутентичная итальянская кухня с видом на процесс приготовления",
    likes: 67,
    views: 203
  },
  {
...
const CATEGORIES = ["Все", "Интерьер", "Кухня", "Обеденная зона", "Вино", "Бар", "События"];
            ...
            <DialogTrigger asChild>
              <Card className="group cursor-pointer overflow-hidden hover-scale">
                <CardContent className="p-0 relative">
                  <div className={`relative ${viewMode === "masonry" ? "mb-4" : "aspect-[4/3]"} overflow-hidden`}>
                    <img
                      src={image.src}
                      alt={image.alt}
              ...
              <div className="relative w-full h-full flex flex-col">
                ...
                <DialogHeader className="p-4 border-b">
                  <div className="flex items-center justify-between">
                    <div>
                      <DialogTitle className="text-xl">{filteredImages[selectedImage]?.title}</DialogTitle>
                      <div className="flex items-center gap-4 text-sm text-muted-foreground mt-1">
                        {filteredImages[selectedImage]?.date && (
                          <span className="flex items-center gap-1">
                            <Calendar className="w-3 h-3" />
                            {new Date(filteredImages[selectedImage].date!).toLocaleDateString('ru-RU')}
                          </span>
                        )}
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        title={isSlideshow ? "Остановить слайд-шоу" : "Запустить слайд-шоу"}
                      >
                      ...
                      <Button
                        ...
                        title={likedImages.has(filteredImages[selectedImage].id) ? "Убрать из избранного" : "Добавить в избранное"}
                      >
                        <Heart className={`w-4 h-4 ${likedImages.has(filteredImages[selectedImage].id) ? 'text-red-500 fill-current' : ''}`} />
                      </Button>
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            ...
            <div className="flex items-center gap-4">
              ...
              {isSlideshow && <Badge variant="secondary">Слайд-шоу активно</Badge>}
            </div>
          </div>
        </div>

        {/* Галерея */}
        {filteredImages.length > 0 ? renderImageGrid() : (
