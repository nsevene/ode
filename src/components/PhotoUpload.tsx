  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (error.message?.includes('Bucket not found')) {
        toast.error('Хранилище фотографий не настроено. Обратитесь к администратору.');
    ...
    return (
      <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
        <div className="flex flex-col items-center space-y-2">
          ...
          <p className="text-gray-500">Войдите в систему, чтобы загружать фотографии</p>
        </div>
      </div>
    );
  }
      ...
      >
        ...
        <div className="flex flex-col items-center space-y-2">
          ...
          <p className="text-sm text-gray-400">
            Поддерживаются: JPG, PNG, GIF (до {maxFiles} файлов, максимум 5MB каждый)
          </p>
        </div>
      </div>
