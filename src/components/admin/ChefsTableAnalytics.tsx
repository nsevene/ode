...
const ChefsTableAnalytics = () => {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const bookingChartData = Object.entries(bookingsByDay).map(([date, count]) => ({
        date: format(new Date(date), 'dd MMM', { locale: ru }),
        bookings: Number(count)
      }));

      const guestChartData = Object.entries(guestsByDay).map(([date, count]) => ({
        date: format(new Date(date), 'dd MMM', { locale: ru }),
        guests: Number(count)
      }));

      // Calculate metrics
      const totalRevenue = bookings?.reduce((sum, booking) => sum + (Number(booking.payment_amount) || 0), 0) || 0;
      const totalBookings = bookings?.length || 0;
      const totalGuests = bookings?.reduce((sum, booking) => sum + Number(booking.guest_count), 0) || 0;
      const occupancyRate = totalGuests > 0 ? (totalGuests / (totalBookings * 30)) * 100 : 0;
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">${topMetrics.totalRevenue.toFixed(2)}</p>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{topMetrics.occupancyRate}%</p>
                <div className="flex items-center text-sm text-green-600">
                  <TrendingUp className="h-4 w-4 mr-1" />
                  +5.2%
                </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">${topMetrics.totalBookings > 0 ? (topMetrics.totalRevenue / topMetrics.totalBookings).toFixed(2) : '0'}</p>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-green-50 rounded-lg">
              <h4 className="font-semibold text-green-800">Хорошие показатели</h4>
            ...
            <div className="p-4 bg-yellow-50 rounded-lg">
              <h4 className="font-semibold text-yellow-800">Требует внимания</h4>
