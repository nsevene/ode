...
const DocumentsManagement = lazy(() => import("@/components/admin/DocumentsManagement"));
...
const mockAnalyticsData = {
  ...
  experiences: [
    ...
    { name: 'Lounge Experience', bookings: 67, percentage: 11, revenue: 2010000 },
  ],
  dailyActivity: [
...
export default function EnhancedAdminDashboard() {
  ...
  const quickActions: QuickAction[] = [
    {
      ...
      icon: CalendarDays,
      action: () => toast({ title: "Перенаправление...", description: "Переход к созданию события" }),
      variant: 'default'
    },
    {
      ...
      icon: Bell,
      action: () => toast({ title: "Отправлено", description: "Уведомление отправлено пользователям" }),
      variant: 'outline'
    },
    {
      ...
      icon: Download,
      action: () => toast({ title: "Экспорт", description: "Начата выгрузка данных" }),
      variant: 'outline'
    },
    {
      title: 'Очистить кэш',
      ...
      icon: RefreshCw,
      action: () => toast({ title: "Очищено", description: "Кэш системы очищен" }),
      variant: 'destructive'
    }
  ];

  const systemAlerts = [
    { type: 'warning', message: 'Высокая нагрузка на сервер (89% CPU)', time: '2 мин назад' },
    { type: 'info', message: 'Планируется обновление в 02:00', time: '1 час назад' },
    { type: 'success', message: 'Резервное копирование завершено', time: '3 часа назад' }
  ];
  ...
  return (
    <div className={`container mx-auto py-8 px-4 ${isMobile ? 'space-y-4' : 'space-y-6'}`}>
      {/* Заголовок с действиями */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold">Административная панель</h1>
          <p className="text-muted-foreground">Управление и аналитика ODE Food Hall</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" size="sm" onClick={loadRealTimeData} disabled={loading}>
      ...
      <Tabs defaultValue="analytics" className="space-y-6">
        <TabsList className={`grid w-full ${isMobile ? 'grid-cols-4' : 'grid-cols-10'}`}>
          ...
          {!isMobile && <TabsTrigger value="notifications">Уведомления</TabsTrigger>}
        ...
        <TabsContent value="analytics" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Бронирования и выручка по месяцам</CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={mockAnalyticsData.bookings}>
                    ...
                    <Area yAxisId="left" type="monotone" dataKey="count" stackId="1" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.6} />
                    <Area yAxisId="right" type="monotone" dataKey="revenue" stackId="2" stroke="#06b6d4" fill="#06b6d4" fillOpacity={0.6} />
                  </AreaChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Распределение бронирований по типам</CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Распределение бронирований и посетителей по часам</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <div key={index} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/30 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{exp.bookings} бронирований</p>
                      </div>
                    </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">Модуль управления пользователями</h3>
                <p className="text-muted-foreground">Этот модуль находится в разработке</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {realTimeEvents.map((event, index) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/30 transition-colors">
                    ...
                    <div className="text-right">
                      <Badge variant="outline">{event.properties?.page || 'Неизвестно'}</Badge>
                      {event.user_id && (
                ...
                {realTimeEvents.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">Нет недавних событий</p>
                  </div>
                )}
              </div>
            </CardContent>
