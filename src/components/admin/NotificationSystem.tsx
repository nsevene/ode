...
const NotificationSystem = () => {
    ...
    const mockNotifications: Notification[] = [
      {
        ...
        message: 'Получено новое бронирование от Анна Петрова на винную дегустацию',
        read: false,
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'new_booking': return <User className="h-4 w-4 text-blue-600" />;
      case 'payment_received': return <Check className="h-4 w-4 text-green-600" />;
      case 'booking_cancelled': return <Clock className="h-4 w-4 text-red-600" />;
                    ...
                    {notification.booking_details && (
                      <div className="text-xs text-muted-foreground bg-white/50 p-2 rounded border">
                        <div className="grid grid-cols-2 gap-2">
                          <span><strong>Гость:</strong> {notification.booking_details.guest_name}</span>
                          <span><strong>Сумма:</strong> {(notification.booking_details.payment_amount / 100).toLocaleString('ru-RU')}₽</span>
                          <span><strong>Дата:</strong> {format(new Date(notification.booking_details.booking_date), 'dd.MM.yyyy', { locale: ru })}</span>
                          <span><strong>Опыт:</strong> {notification.booking_details.experience_type}</span>
                        </div>
                      </div>
                    )}
