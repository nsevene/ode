...
const MenuManagement: React.FC = () => {
  ...
  const categories = [
    ...
    'Salads', 'Soups', 'Pasta', 'Pizza', 'Sandwiches', 'Other'
  ];
  ...
  const loadVendors = async () => {
    try {
      ...
      setVendors(data?.filter(v => v.is_active).sort((a, b) => a.name.localeCompare(b.name)) || []);
    } catch (error) {
  ...
  const loadMenuItems = async (vendorId?: string) => {
    try {
      ...
      const mappedData = data?.map(item => ({ ...item, vendor_name: (item as any).vendors?.name || 'N/A' }));
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  {formErrors?.formErrors.fieldErrors.name && (
                    <p className="text-red-500 text-sm mt-1">{formErrors.formErrors.fieldErrors.name}</p>
                  )}
                </div>
                <div>
                  ...
                  {formErrors?.formErrors.fieldErrors.vendor_id && (
                    <p className="text-red-500 text-sm mt-1">{formErrors.formErrors.fieldErrors.vendor_id}</p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  {formErrors?.formErrors.fieldErrors.category && (
                    <p className="text-red-500 text-sm mt-1">{formErrors.formErrors.fieldErrors.category}</p>
                  )}
                </div>
                <div>
                  <Label htmlFor="preparation_time">Время приготовления (мин)</Label>
                  <Input
                    id="preparation_time"
                    type="number"
                  ...
                  {formErrors?.formErrors.fieldErrors.preparation_time && (
                    <p className="text-red-500 text-sm mt-1">{formErrors.formErrors.fieldErrors.preparation_time}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: any) => setFormData({ ...formData, spice_level: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                ...
                {formErrors?.formErrors.fieldErrors.spice_level && (
                  <p className="text-red-500 text-sm mt-1">{formErrors.formErrors.fieldErrors.spice_level}</p>
                )}
              </div>
