...
import { Eye, MessageSquare, CheckCircle, XCircle, Clock, Filter, Search, Download, FileText, ExternalLink } from 'lucide-react';
          ...
          <Card key={application.id}>
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription>
                    {application.contact_person} • {application.email} • {application.cuisine_type}
                  </CardDescription>
                </div>
            ...
            <CardContent>
              <div className="space-y-2 mb-4">
                <p><strong>Сектор:</strong> {application.preferred_sector}</p>
                <p><strong>Опыт:</strong> {application.experience_years || 'Не указан'} лет</p>
                <p><strong>Инвестиции:</strong> {application.investment_budget || 'Не указан'}</p>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm">{selectedApplication.cuisine_type}</p>
                          </div>
                        </div>

                        <div>
                          <Label>Описание концепции</Label>
                          <p className="text-sm mt-1 p-3 bg-gray-50 rounded">{selectedApplication.description}</p>
                        </div>
                            ...
                            <div className="mt-2 space-y-2">
                              {selectedApplication.documents.map((docUrl, index) => (
                                <div key={index} className="flex items-center gap-2 p-2 border rounded">
                                  ...
                                  <span className="text-sm flex-1">Документ {index + 1}</span>
                                  <Button
                                    variant="ghost"
                                    size="sm"
