...
const DOCUMENT_TYPES = [
  ...
  { value: 'invoice', label: 'Счет-фактура', color: 'bg-green-100 text-green-800' },
  { value: 'report', label: 'Отчет', color: 'bg-purple-100 text-purple-800' },
  { value: 'legal', label: 'Юридический', color: 'bg-red-100 text-red-800' },
  { value: 'financial', label: 'Финансовый', color: 'bg-yellow-100 text-yellow-800' },
  { value: 'administrative', label: 'Административный', color: 'bg-gray-100 text-gray-800' },
...
const DocumentsManagement = () => {
  ...
  const handleUpdateMetadata = async (doc: Document, updates: Partial<Document>) => {
    try {
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select value={uploadForm.bucket} onValueChange={(value) => setUploadForm({...uploadForm, bucket: value})}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{documents.filter(d => d.is_signed).length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{documents.filter(d => d.document_type === 'contract').length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground">
                          ...
                          <span>
                            {format(new Date(doc.created_at), 'dd.MM.yyyy HH:mm', { locale: ru })}
                          </span>
                          <span className="flex items-center gap-1">
                            <FolderOpen className="h-3 w-3" />
                            {STORAGE_BUCKETS.find(b => b.id === doc.bucket_name)?.name || doc.bucket_name}
                          </span>
                        </div>
                            ...
                            <div className="space-y-4">
                              <div>
                                ...
                                <Select 
                                  ...
                                  onValueChange={(value) => setEditingDoc({...doc, document_type: value})}
                                >
                                  <SelectTrigger>
                                    <SelectValue />
                                  </SelectTrigger>
                              ...
                              <div>
                                ...
                                <Textarea
                                  ...
                                  onChange={(e) => setEditingDoc({...doc, description: e.target.value})}
                                />
                              </div>
                              ...
                              <div>
                                ...
                                <Input
                                  ...
                                  onChange={(e) => setEditingDoc({...doc, tags: e.target.value.split(',').map(t => t.trim())})}
                                />
                              </div>
                              
                              <Button 
                                onClick={() => editingDoc && handleUpdateMetadata(doc, editingDoc)}
                                className="w-full"
                              >
                                Сохранить изменения
                              </Button>
