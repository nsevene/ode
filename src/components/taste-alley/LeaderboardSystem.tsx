...
const LeaderboardSystem = ({ currentUser }: LeaderboardSystemProps) => {
  ...
  const currentUserEntry = leaderboard.find(entry => entry.id === currentUser.id);
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-lg font-semibold">{currentUserEntry.name}</h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    <div className="font-semibold text-emerald-600">#{currentUserEntry.rank}</div>
                    <div className="text-muted-foreground">Rank</div>
                  </div>
                  <div>
                    <div className="font-semibold text-emerald-600">{currentUserEntry.score}</div>
                    <div className="text-muted-foreground">Score</div>
                  </div>
                  <div>
                    <div className="font-semibold text-emerald-600">{currentUserEntry.completedSectors}</div>
                    <div className="text-muted-foreground">Sectors</div>
                  </div>
                  <div>
                    <div className="font-semibold text-emerald-600">{currentUserEntry.fastestTime}min</div>
      ...
      <Tabs defaultValue="overall" className="w-full">
        ...
        <TabsContent value="overall" className="space-y-4">
          ...
          <div className="space-y-3">
            {leaderboard.slice(0, 10).map((entry) => (
              ...
              >
                {entry.rank <= 3 && (
                  <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${getRankColor(entry.rank)}`} />
                )}
        ...
        <TabsContent value="speed">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {[...leaderboard].sort((a, b) => a.fastestTime - b.fastestTime).slice(0, 10).map((entry, index) => (
                <Card key={entry.id} className={entry.id === currentUser.id ? 'ring-2 ring-emerald-500' : ''}>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4">
        ...
        <TabsContent value="completion">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {[...leaderboard].sort((a, b) => b.completedSectors - a.completedSectors).slice(0, 10).map((entry, index) => (
                <Card key={entry.id} className={entry.id === currentUser.id ? 'ring-2 ring-emerald-500' : ''}>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4">
                      <span className="font-bold text-lg">#{index + 1}</span>
        ...
        <TabsContent value="achievements">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {[...leaderboard].sort((a, b) => b.achievements - a.achievements).slice(0, 10).map((entry, index) => (
                <Card key={entry.id} className={entry.id === currentUser.id ? 'ring-2 ring-emerald-500' : ''}>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4">
                      <span className="font-bold text-lg">#{index + 1}</span>
