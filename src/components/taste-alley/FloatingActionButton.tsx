...
const FloatingActionButton = ({ onHelpClick, onShareClick, onRewardsClick }: FloatingActionButtonProps) => {
  const [isExpanded, setIsExpanded] = useState(false);

  const actions = [
    { icon: HelpCircle, label: "Help", onClick: onHelpClick, color: "bg-blue-500 hover:bg-blue-600" },
    { icon: Share2, label: "Share", onClick: onShareClick, color: "bg-green-500 hover:bg-green-600" },
    { icon: Gift, label: "Rewards", onClick: onRewardsClick, color: "bg-purple-500 hover:bg-purple-600" }
  ];
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <div className="flex flex-col-reverse items-center space-y-reverse space-y-3">
        ...
        >
          {isExpanded ? <ChevronUp className="h-6 w-6" /> : <Zap className="h-6 w-6" />}
        </Button>
      </div>
    </div>
  );
