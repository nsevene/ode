...
import { 
  Trophy, Users, Share2, Gift, Smartphone, 
  Bell, Star, Target, Zap, Calendar, Gamepad2 
} from "lucide-react";
...
const GamifiedQuestProgress = () => {
  ...
  const completedQuests = quests.filter(q => getQuestProgress(q.id)?.completed).length;
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>Level</span>
                        <Badge variant="outline">{progress?.user_level || 1}</Badge>
                      </div>
                      <div className="flex justify-between">
                        <span>Current Sector</span>
                        <Badge variant="outline">{progress?.current_sector || 0}/4</Badge>
                      </div>
                      <div className="flex justify-between">
                        <span>Total Score</span>
                        <Badge variant="outline">{progress?.total_score || 0}</Badge>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      {progress?.last_activity_date ? (
                        <div className="text-sm text-muted-foreground">
                          Last activity: {new Date(progress.last_activity_date).toLocaleDateString()}
                        </div>
                      ) : (
                        <div className="text-sm text-muted-foreground">
                          No recent activity
