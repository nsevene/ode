...
const EnhancedRewardSystem = () => {
  ...
  const categories = [
    ...
    { id: 'experience', name: 'Experiences', icon: <Star className="h-4 w-4" /> },
        ...
        {filteredRewards.map(reward => (
          <Card 
            ...
            className={`relative overflow-hidden ${
              canAfford(reward.pointsRequired) ? 'ring-2 ring-emerald-500' : 'opacity-75'
            }`}
          >
            <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${getRarityColor(reward.rarity)}`} />
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <Badge variant="outline" className={`${getRarityBadge(reward.rarity)} text-xs`}>
                      {reward.rarity}
                    </Badge>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-bold text-emerald-600">{reward.pointsRequired}</div>
            ...
            <CardContent className="pt-0">
              ...
              {!canAfford(reward.pointsRequired) && (
                <div className="mb-3">
                  <div className="flex justify-between text-xs mb-1">
                    <span>Progress</span>
                    <span>{progress?.total_score || 0}/{reward.pointsRequired}</span>
                  </div>
                  <Progress 
                    value={((progress?.total_score || 0) / reward.pointsRequired) * 100} 
                    className="h-2"
                  />
                </div>
              )}
              ...
              <Button 
                ...
                variant={canAfford(reward.pointsRequired) ? 'default' : 'outline'}
              >
                {canAfford(reward.pointsRequired) ? 'Claim Reward' : 'Not Enough Points'}
              </Button>
            </CardContent>
          </Card>
        ))}
