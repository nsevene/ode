...
import { 
  Share2, Facebook, Twitter, Instagram, Copy, Download, QrCode, 
  Camera, Heart, MessageCircle, Users, Sparkles 
} from "lucide-react";
...
const EnhancedSocialSharing = ({ achievement, userStats, questProgress, trigger }: EnhancedSocialSharingProps) => {
  ...
  const socialTemplates: SocialTemplate[] = [
    {
      ...
      preview: (
        <div className="bg-gradient-to-r from-purple-500 to-pink-500 p-4 rounded-lg text-white">
          ...
          <p className="text-sm opacity-90">Just completed my Taste Alley quest!</p>
        </div>
      ),
      hashtags: ['#TasteAlley', '#Achievement', '#ODEFoodHall', '#CulinaryQuest']
    },
    {
      id: 'progress',
      ...
      preview: (
        <div className="bg-gradient-to-r from-blue-500 to-indigo-500 p-4 rounded-lg text-white">
          ...
          <p className="text-sm opacity-90">Now level {userStats?.level || 1} at Taste Alley</p>
        </div>
      ),
    ...
    },
    {
      ...
      preview: (
        <div className="bg-gradient-to-r from-orange-500 to-red-500 p-4 rounded-lg text-white">
          ...
          <p className="text-sm opacity-90">Ranked #{userStats?.rank || 1} in Taste Alley</p>
        </div>
      ),
      hashtags: ['#TasteAlley', '#Leaderboard', '#TopPerformer', '#ODEFoodHall']
    }
  ];
  ...
  const platforms = [
    ...
    { id: 'tiktok', name: 'TikTok', icon: <MessageCircle className="h-4 w-4" />, color: 'bg-black' }
  ];
              ...
              {platforms.map(platform => (
                <Button
                  ...
                  className={`h-20 flex-col gap-2 ${
                    selectedPlatform === platform.id ? 'ring-2 ring-emerald-500' : ''
                  }`}
