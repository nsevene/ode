...
const AnimatedSectorCard = ({
  ...
  const getButtonStyles = () => {
    if (isCompleted) return 'bg-forest-green hover:bg-forest-green/80 text-white';
    ...
    >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                {isCompleted && (
                  <Badge variant="success" className="bg-forest-green text-white animate-pulse">
                    <CheckCircle className="h-3 w-3 mr-1" />
                    Completed
                  </Badge>
                )}
                {isActive && (
                  <Badge variant="warning" className="bg-gold-accent text-white animate-pulse">
                    <Play className="h-3 w-3 mr-1" />
                    Active
                  </Badge>
                )}
                {!isUnlocked && (
                  <Badge variant="outline" className="text-charcoal/60 border-charcoal/30">
      ...
      <CardContent className="space-y-4 relative z-10">
        ...
        <div className="space-y-2">
          <h5 className="text-sm font-semibold text-foreground">Experiences:</h5>
          <div className="grid grid-cols-1 gap-1">
            {experiences.map((experience, index) => (
              <div
                key={index}
                className={`flex items-center gap-2 text-xs p-2 rounded-md transition-all duration-500 ${
                  isHovered ? 'transform translate-x-2' : 'transform translate-x-0'
                } ${
                  isCompleted
              ...
              >
                <div className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  ...
                  isActive ? 'bg-gold-accent' : 
                  isUnlocked ? 'bg-sage-blue' : 'bg-charcoal/40'
                } ${isHovered ? 'scale-125' : 'scale-100'}`} />
                <span className="transition-all duration-300">{experience}</span>
              </div>
            ))}
          </div>
        </div>
