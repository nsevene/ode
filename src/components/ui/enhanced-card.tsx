...
const EnhancedCard = React.forwardRef<HTMLDivElement, EnhancedCardProps>(
  ({ className, variant = 'default', size = 'md', hover = false, loading = false, interactive = false, children, ...props }, ref) => {
    const cardVariants = {
    ...
    const hoverEffects = hover ? "hover:shadow-medium hover:scale-[1.02] transition-all duration-300" : "";
    const interactiveEffects = interactive ? "cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/20" : "";

    if (loading) {
      return (
        <Card ref={ref} className={cn(cardVariants[variant], cardSizes[size], className)} {...props}>
          <CardHeader>
...
const EnhancedCardContent = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & { spacing?: 'none' | 'sm' | 'md' | 'lg' }
