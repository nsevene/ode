  ...
  const experiences = [
    {
      ...
      price: 450000,
      description: "Intimate wine journey through our cellar with expert sommelier guidance",
    ...
    },
    {
      ...
      price: 300000,
      description: "Exclusive second-floor experience with live entertainment and premium service",
    ...
    },
    {
      ...
      price: 650000,
      description: "Exclusive multi-course dining experience with chef interaction",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {experiences.map((experience, index) => (
            <Card key={index} className="group hover:shadow-soft transition-all duration-300 overflow-hidden h-full flex flex-col">
              <div className="aspect-[4/3] relative overflow-hidden">
                ...
                <div className="absolute top-4 right-4">
                  <div className="flex items-center gap-1 bg-pure-white/90 rounded-full px-2 py-1">
                    ...
                    <span className="text-xs font-medium text-charcoal">{experience.rating}</span>
                  </div>
                </div>
              </div>
              ...
              <CardContent className="p-6 flex-1 flex flex-col">
                <div className="mb-4">
                  <h3 className="font-bold text-charcoal mb-2 text-lg">{experience.name}</h3>
                  <p className="text-sm text-charcoal/70 leading-relaxed">{experience.description}</p>
                </div>
                  ...
                  <div className="flex justify-between items-center mb-4">
                    {typeof experience.price === 'number' ? (
                      <>
                        ...
                        <Price value={experience.price} showWords className="font-semibold text-charcoal" />
                      </>
                    ) : (
                      <span className="font-semibold text-charcoal">{experience.price}</span>
                    )}
                    <span className="text-xs text-charcoal/70">per person</span>
                  </div>
                  <Button asChild className="w-full bg-sage hover:bg-sage/90 text-pure-white" size="sm">
