...
interface CrossSystemEvent {
  type: 'loyalty_upgrade' | 'achievement_unlocked' | 'social_milestone' | 'special_offer' | 'quest_completed';
  title: string;
  message: string;
  data?: any;
...
export const CrossSystemNotifications = () => {
      ...
      if (loyaltyData.tier === 'platinum' && posts.length >= 5) {
        ...
        if (!processedEvents.has(eventKey)) {
          const event: CrossSystemEvent = {
            ...
            message: 'Как VIP-гость и активный участник сообщества, вы получили доступ к приватному Chef\'s Table!',
            data: { 
  ...
  const handleRealtimeNotification = (notification: any) => {
    const icon = getEventIcon(notification.data?.type || 'achievement_unlocked');
  ...
  const triggerEventActions = (event: CrossSystemEvent) => {
    ...
    switch (event.type) {
      case 'loyalty_upgrade':
        ...
        window.dispatchEvent(new CustomEvent('loyaltyUpgrade', { 
          detail: event.data 
        }));
        break;
      ...
      case 'social_milestone':
        ...
        window.dispatchEvent(new CustomEvent('socialMilestone', { 
          detail: event.data 
        }));
        break;
      ...
      case 'special_offer':
        ...
        window.dispatchEvent(new CustomEvent('specialOffer', { 
          detail: event.data 
        }));
        break;
    }
...
const getEventIcon = (type: string) => {
  switch (type) {
    case 'loyalty_upgrade': return Trophy;
    case 'achievement_unlocked': return Badge;
    case 'social_milestone': return Users;
    case 'special_offer': return Gift;
    case 'quest_completed': return Star;
