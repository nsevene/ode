...
export const DataSynchronization = () => {
  ...
  const setupSyncChannel = (tableName: string, handler: (payload: any) => void) => {
    return supabase
  ...
  const handleLoyaltySync = async (payload: any) => {
    ...
    if (eventType === 'UPDATE' && oldRecord && newRecord.tier !== oldRecord.tier) {
  ...
  const processSyncQueue = async () => {
    ...
    try {
      ...
      const groupedEvents = queue.reduce((groups, event) => {
  ...
  const checkTasteCompassCompletion = async (userId: string) => {
    ...
    if (count && count >= 6) { // Все 6 секторов
      ...
      await supabase.from('user_notifications').insert({
        ...
        message: 'За прохождение всех секторов Taste Compass вы получили доступ к эксклюзивному Chef\'s Table!',
        data: { 
