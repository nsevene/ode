...
export default function PolicyBadges(p: Props) {
  const items: { label: string; variant?: 'default' | 'secondary' | 'outline' | 'warning' }[] = [];
  
  if (p.acHalal) items.push({ label: 'AC Halal · 0% ABV', variant: 'default' });
  if (p.alcoholAllowed) items.push({ label: 'Alcohol allowed', variant: 'warning' });
  if (p.bottlesOnly) items.push({ label: 'Bottles only · on‑premise', variant: 'outline' });
  ...
  if (p.serviceFee5) items.push({ label: 'Service fee 5%', variant: 'secondary' });
