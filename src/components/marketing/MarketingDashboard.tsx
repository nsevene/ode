...
interface MarketingCampaign {
  id: string;
  name: string;
  type: 'birthday' | 'loyalty_tier' | 'first_booking' | 'return_customer' | 'seasonal' | 'abandoned_cart';
...
export const MarketingDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (campaignResponse.data) setCampaigns(campaignResponse.data as MarketingCampaign[]);
    } catch (error) {
  ...
  const generatePromoCode = async () => {
    try {
      const { data, error } = await supabase.rpc('generate_promo_code', { prefix: 'MARKETING' });
      if (error) throw error;
      return data;
    } catch (error) {
      console.error('Error generating promo code:', error);
      return 'MARKETING' + Math.random().toString(36).substr(2, 6).toUpperCase();
    }
  };
  ...
  const createPromoCode = async (formData: Partial<PromoCode>) => {
    try {
      ...
      track('promo_code_created', { code, discount_type: formData.discount_type });
      loadData();
    ...
    <div className="container mx-auto py-8 px-4 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Управление промо-кодами, кампаниями и программой лояльности</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Активные промо-коды</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{campaigns.filter(c => c.is_active).length}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{promoCodes.reduce((sum, p) => sum + p.usage_count, 0)}</div>
            <p className="text-xs text-muted-foreground">
              Всего активаций
            </p>
          </CardContent>
                    ...
                    <TableRow key={promo.id}>
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          ...
                          {promo.usage_limit && (
                            <Progress value={getUsageProgress(promo.usage_count, promo.usage_limit)} />
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
              ...
              <Card key={campaign.id} className={campaign.is_active ? 'border-green-200' : 'border-gray-200'}>
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      <span>Скидка:</span>
                      <span>{formatDiscount(campaign.discount_type, campaign.discount_value)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Период:</span>
...
// Form components
const PromoCodeForm: React.FC<{ onSubmit: (data: Partial<PromoCode>) => void }> = ({ onSubmit }) => {
        ...
        <div>
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="return_customer">Возвращающийся клиент</SelectItem>
