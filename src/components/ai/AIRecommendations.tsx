...
const AIRecommendations = () => {
  ...
  const getRecommendationIcon = (type: string) => {
    switch (type) {
      case 'dish': return <ChefHat className="h-5 w-5" />;
      case 'vendor': return <Store className="h-5 w-5" />;
      case 'event': return <Calendar className="h-5 w-5" />;
      case 'experience': return <Sparkles className="h-5 w-5" />;
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Анализируем ваши предпочтения...</p>
          </div>
        </CardContent>
      </Card>
    );
    ...
    <div className="max-w-6xl mx-auto space-y-6 p-4">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">
          Персонализированные рекомендации на основе ваших предпочтений и поведения
        </p>
      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="text-center">
                ...
                <p className="font-medium">{insights.userProfile.foodPersonality}</p>
              </div>
              <div className="text-center">
                <p className="text-sm text-muted-foreground">Риск</p>
                <p className="font-medium">{insights.userProfile.riskTaker ? 'Любитель риска' : 'Консерватор'}</p>
              </div>
              <div className="text-center">
                <p className="text-sm text-muted-foreground">Социальность</p>
                <p className="font-medium">{insights.userProfile.socialDiner ? 'Социальный' : 'Индивидуальный'}</p>
              </div>
              <div className="text-center">
                <p className="text-sm text-muted-foreground">Здоровье</p>
                <p className="font-medium">{insights.userProfile.healthConscious ? 'Сознательный' : 'Расслабленный'}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-center">
                  ...
                  <p className="text-gray-500">Нет рекомендаций по выбранным фильтрам</p>
                </div>
              </CardContent>
            </Card>
          ) : (
                ...
                <Card key={recommendation.id} className="relative">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        onClick={() => handleRateRecommendation(recommendation.id, 5)}
                        className="flex-1"
                      >
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleRateRecommendation(recommendation.id, 1)}
                        className="flex-1"
                      >
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Персонализированных</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-accent">
                {getRecommendationsByConfidence(0.8).length}
              </p>
              <p className="text-sm text-muted-foreground">Высокой уверенности</p>
            </div>
            <div>
