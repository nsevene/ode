...
export const MenuSchema = ({ 
  ...
  const schema = {
    ...
    "description": "Полное меню ресторана ODE Food Hall с блюдами от лучших шефов мира",
    "inLanguage": "ru",
    "hasMenuSection": menuItems.reduce((sections: any[], item) => {
      ...
      section.hasMenuItem.push({
        ...
        "image": item.image,
        "suitableForDiet": item.allergens?.map(allergen => {
          switch(allergen.toLowerCase()) {
            ...
            case 'dairy-free': return 'https://schema.org/DiabeticDiet';
            default: return null;
          }
        }).filter(Boolean),
