  ...
  return (
    <form onSubmit={onSubmit} className="mx-auto max-w-xl space-y-4">
      ...
      <div className="rounded-xl border p-4">
        <div className="text-xs uppercase text-muted-foreground mb-2">Формат</div>
        <div className="flex gap-4">
          <label className="flex items-center gap-2">
            <input type="radio" name="format" checked={form.format==="long-term"} onChange={()=>setForm({...form, format:"long-term"})}/>
            <span>Долгосрочный резидент</span>
          </label>
          <label className="flex items-center gap-2">
            <input type="radio" name="format" checked={form.format==="popup"} onChange={()=>setForm({...form, format:"popup"})}/>
...
function Input({label,value,onChange,required=false,placeholder="",type="text"}:{label:string;value:string;onChange:(v:string)=>void;required?:boolean;placeholder?:string;type?:string}) {
  return (
    <label className="block">
      <div className="mb-1 text-sm">{label}{required && <span className="text-red-500">*</span>}</div>
      <input 
...
function Textarea({label,value,onChange}:{label:string;value:string;onChange:(v:string)=>void}) {
  return (
    <label className="block">
      ...
      <textarea className="w-full rounded-lg border px-3 py-2 min-h-[120px]" value={value} onChange={(e)=>onChange(e.target.value)} />
    </label>
  );
}