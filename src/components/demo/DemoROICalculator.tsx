...
import { Calculator, TrendingUp, DollarSign, Calendar, BarChart } from 'lucide-react';
import { track } from '@/lib/analytics';

const DemoROICalculator = () => {
  const [scenario, setScenario] = useState<'conservative' | 'ambitious' | 'custom'>('conservative');
  ...
  const handleScenarioChange = (newScenario: 'conservative' | 'ambitious' | 'custom') => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold text-charcoal-dark mb-3">ROI Calculator Demo</h2>
        <p className="text-charcoal-medium max-w-2xl mx-auto">
          Model your investment returns with real market scenarios. Adjust parameters to see 
          how different variables affect your payback period and ROI.
        </p>
      </div>

      {/* Scenario Selector */}
      <div className="flex justify-center gap-2">
        {(['conservative', 'ambitious', 'custom'] as const).map((scenarioOption) => (
          <Button
            key={scenarioOption}
          ...
          >
            {scenarioOption === 'conservative' && <TrendingUp className="w-4 h-4 mr-2" />}
            {scenarioOption === 'ambitious' && <BarChart className="w-4 h-4 mr-2" />}
            {scenarioOption === 'custom' && <Calculator className="w-4 h-4 mr-2" />}
            {scenarioOption}
          </Button>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-pure-white/80 backdrop-blur">
          <CardContent className="p-6">
            <h3 className="text-lg font-semibold text-charcoal-dark mb-4">Investment Parameters</h3>
              ...
              <div>
                ...
                {scenario === 'custom' && (
                  <Slider
                    ...
                    onValueChange={(value) => handleCustomValueChange('spaceSize', value)}
                    max={50}
                    min={8}
                    step={1}
                    className="w-full"
                ...
                {scenario !== 'custom' && (
                  <div className="w-full bg-cream-light h-2 rounded-full">
                    <div 
                      ...
                      style={{ width: `${(getValues().spaceSize / 50) * 100}%` }}
                    />
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {scenario === 'custom' && (
                  <Slider
                    ...
                    onValueChange={(value) => handleCustomValueChange('capex', value)}
                    max={25000000}
                    min={3000000}
                    step={500000}
                    className="w-full"
                ...
                {scenario !== 'custom' && (
                  <div className="w-full bg-cream-light h-2 rounded-full">
                    <div 
                      className="bg-burgundy-primary h-2 rounded-full"
                      style={{ width: `${(getValues().capex / 25000000) * 100}%` }}
                    />
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {scenario === 'custom' && (
                  <Slider
                    ...
                    onValueChange={(value) => handleCustomValueChange('avgTicket', value)}
                    max={300000}
                    min={80000}
                    step={10000}
                    className="w-full"
                ...
                {scenario !== 'custom' && (
                  <div className="w-full bg-cream-light h-2 rounded-full">
                    <div 
                      className="bg-burgundy-primary h-2 rounded-full"
                      style={{ width: `${((getValues().avgTicket - 80000) / 220000) * 100}%` }}
                    />
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {scenario === 'custom' && (
                  <Slider
                    ...
                    onValueChange={(value) => handleCustomValueChange('dailyCustomers', value)}
                    max={80}
                    min={10}
                    step={1}
                    className="w-full"
                ...
                {scenario !== 'custom' && (
                  <div className="w-full bg-cream-light h-2 rounded-full">
                    <div 
                      className="bg-burgundy-primary h-2 rounded-full"
                      style={{ width: `${(getValues().dailyCustomers / 80) * 100}%` }}
                    />
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {scenario === 'custom' && (
                  <Slider
                    ...
                    onValueChange={(value) => handleCustomValueChange('revenueShare', value)}
                    max={25}
                    min={10}
                    step={1}
                    className="w-full"
                ...
                {scenario !== 'custom' && (
                  <div className="w-full bg-cream-light h-2 rounded-full">
                    <div 
                      className="bg-burgundy-primary h-2 rounded-full"
                      style={{ width: `${((getValues().revenueShare - 10) / 15) * 100}%` }}
                    />
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-pure-white/10 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm text-burgundy-light">Monthly Revenue</span>
                </div>
                <div className="text-2xl font-bold">{formatIDR(results.monthlyRevenue)}</div>
              </div>

              <div className="bg-pure-white/10 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  <TrendingUp className="w-4 h-4" />
                  <span className="text-sm text-burgundy-light">Your Share ({getValues().revenueShare}%)</span>
                </div>
                <div className="text-2xl font-bold">{formatIDR(results.monthlyRevenueShare)}</div>
              </div>

              <div className="bg-pure-white/10 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  <Calendar className="w-4 h-4" />
                  <span className="text-sm text-burgundy-light">Payback Period</span>
                </div>
              ...
              <div className="bg-mustard-accent/20 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm text-mustard-accent">Annual ROI</span>
                </div>
              ...
              <div className="pt-4 border-t border-pure-white/20">
                <div className="text-xs text-burgundy-light mb-2">ROI Status:</div>
                <Badge 
                  className={`${
                ...
                >
                  {results.annualROI >= 25 ? 'Excellent' : results.annualROI >= 15 ? 'Good' : 'Below Target'}
                </Badge>
              </div>
            </div>
          </CardContent>
