...
const bookingSchema = z.object({
  ...
  guest_count: z.number().min(1, 'Минимум 1 гость').max(20, 'Максимум 20 гостей'),
  special_requests: z.string().optional(),
});

type BookingFormData = z.infer<typeof bookingSchema>;

const SimpleEventBookingModal = ({ event, isOpen, onClose, onBookingSuccess }: SimpleEventBookingModalProps) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-3 pt-4">
          ...
          <Button
            onClick={onSubmit}
            disabled={isSubmitting || event.available_spots === 0 || !formData.guestName || !formData.guestEmail}
            className="flex-1"
          >
            {isSubmitting ? (
              <>
