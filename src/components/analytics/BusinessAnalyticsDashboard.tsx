...
const COLORS = ['hsl(var(--primary))', 'hsl(var(--secondary))', 'hsl(var(--accent))', '#ff7300', '#82ca9d'];
...
export const BusinessAnalyticsDashboard: React.FC = () => {
  ...
  const processExperienceMetrics = (bookings: any[]) => {
    const experienceTypes = ['taste_compass', 'wine_staircase', 'chefs_table', 'lounge'];
    
    const metrics = experienceTypes.map(type => {
      const typeBookings = bookings.filter(b => b.experience_type === type);
      const revenue = typeBookings.reduce((sum, b) => sum + (b.payment_amount || 0), 0);
      const avgRating = 4.5;
  ...
  const processCustomerSegments = (bookings: any[], loyaltyData: any[]) => {
    const segments = [
      { segment: 'VIP клиенты', count: 45, revenue: 12500000, avg_spend: 278000, retention_rate: 85 },
      { segment: 'Постоянные', count: 156, revenue: 28900000, avg_spend: 185000, retention_rate: 72 },
      { segment: 'Новые клиенты', count: 234, revenue: 18700000, avg_spend: 80000, retention_rate: 35 },
      { segment: 'Разовые', count: 89, revenue: 5400000, avg_spend: 61000, retention_rate: 15 }
    ];
  ...
  const groupBookingsByPeriod = (bookings: any[], range: string) => {
    ...
    const grouped: { [key: string]: { revenue: number; bookings: number; date: string } } = {};
    ...
    <div className="container mx-auto py-8 px-4 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Комплексная аналитика и отчетность для ODE Food Hall</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(revenueMetrics.total_revenue)}</div>
            <div className="flex items-center text-xs text-muted-foreground">
              ...
              <span className={revenueMetrics.growth_rate >= 0 ? 'text-green-600' : 'text-red-600'}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{revenueMetrics.bookings_count}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{revenueMetrics.conversion_rate.toFixed(1)}%</div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className={`h-3 w-3 ${i < 4 ? 'text-yellow-400 fill-current' : 'text-muted-foreground'}`} />
              ))}
            </div>
          </CardContent>
        </Card>
                  ...
                  <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{segment.count} клиентов</p>
                      </div>
                    </div>
                    <div className="text-right space-y-1">
                      ...
                      <Badge variant={segment.retention_rate > 70 ? 'default' : 'secondary'}>
              ...
              <div className="space-y-4">
                {experienceMetrics.map((experience, index) => (
                  <div key={index} className="p-4 border rounded-lg space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="font-medium capitalize">{experience.experience_type.replace('_', ' ')}</h3>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{formatCurrency(experience.revenue)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Загрузка</p>
                        <div className="flex items-center gap-2">
                          <Progress value={experience.capacity_utilization} className="flex-1" />
                          <span className="text-xs">{experience.capacity_utilization.toFixed(0)}%</span>
                        </div>
                      </div>
                    </div>
                  </div>
