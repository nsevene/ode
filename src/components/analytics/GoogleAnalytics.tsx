...
export const GoogleAnalytics = ({ measurementId, trackingId }: GoogleAnalyticsProps) => {
  const location = useLocation();
  const gaId = measurementId || trackingId;

  useEffect(() => {
    if (typeof window === 'undefined' || typeof document === 'undefined' || !gaId) return;
...
const getStepNumber = (step: string): number => {
  const steps = ['started', 'date_selected', 'time_selected', 'details_entered', 'payment_initiated', 'completed'];
