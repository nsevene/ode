...
export const HeatmapTracker: React.FC<HeatmapTrackerProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      localStorage.setItem(storageKey, JSON.stringify([...existingData, newDataPoint]));
    };
          ...
          <div className="flex items-center justify-between space-x-2">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-muted-foreground">
                Кликов: {heatmapData.reduce((sum, point) => sum + point.clicks, 0)}
              </span>
            </div>
            ...
            <div className="mt-4 space-y-2">
              ...
              {Object.entries(
                heatmapData.reduce((acc, point) => {
                  if (point.element) {
                    acc[point.element] = (acc[point.element] || 0) + point.clicks;
                  }
                  return acc;
              ...
              .slice(0, 5)
              .map(([element, clicks]) => (
                <div key={element} className="flex justify-between items-center text-xs">
                  <span className="font-mono truncate max-w-xs">{element}</span>
