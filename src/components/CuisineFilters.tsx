...
export const CuisineFilters = ({ onFiltersChange }: CuisineFiltersProps) => {
  ...
  const hasActiveFilters = selectedCuisines.length > 0 || selectedDietary.length > 0 || selectedPriceRange.length > 0;
          ...
          >
            ...
            {hasActiveFilters && (
              <Badge variant="secondary" className="ml-2">
                {selectedCuisines.length + selectedDietary.length + selectedPriceRange.length}
              </Badge>
            )}
          </Button>
              ...
              <div className="flex flex-wrap gap-2">
                {CUISINE_FILTERS.map((cuisine) => (
                  ...
                  >
                    ...
                    {cuisine.name}
                    <span className="ml-1 text-xs opacity-75">({cuisine.count})</span>
                  </Badge>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {PRICE_FILTERS.map((price) => (
                  <Badge
                    key={price.id}
                    variant={selectedPriceRange.includes(price.id) ? "default" : "outline"}
                    className="cursor-pointer hover:scale-105 transition-transform"
