...
interface TimeslotBookingProps {
  eventType?: 'taste-compass' | 'wine-staircase' | 'vip-lounge' | 'chefs-table' | 'taste-alley';
  experienceType?: 'taste-compass' | 'wine-staircase' | 'vip-lounge' | 'chefs-table' | 'taste-alley';
...
const TimeslotBooking = ({ 
  ...
  useEffect(() => {
    if (user) {
      setGuestName(user.user_metadata?.display_name || user.email?.split('@')[0] || "");
  ...
  const loadAvailableDates = async () => {
    try {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        if (isAfter(checkDate, startOfDay(today)) || checkDate.toDateString() === today.toDateString()) {
          ...
          if (data && data.length > 0 && data.some((slot: any) => slot.is_available)) {
  ...
  const handleBooking = async () => {
    ...
    try {
      console.log('Starting booking process...', { selectedDate, selectedTime, guestName, guestEmail });
      
      let passportId = null;
      if (nfcPassportEnabled) {
        console.log('Generating NFC passport ID...');
        const { data: passportData, error: passportError } = await supabase.rpc('generate_nfc_passport_id');
        if (passportError) {
      ...
      const totalAmount = basePrice * guestCount + (nfcPassportEnabled ? 500 : 0);
  ...
  const isFormValid = () => {
    return selectedDate && selectedTime && guestName && guestEmail && guestCount > 0;
  };
