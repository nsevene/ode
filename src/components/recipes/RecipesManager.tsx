...
export const RecipesManager = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Удалить рецепт? Это также удалит все связанные ингредиенты.')) return;
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground mb-3">
                    ...
                    <span>⏱️ {recipe.prep_time_minutes + recipe.cook_time_minutes} мин</span>
                  ...
                  {recipe.total_calories && (
                    <div className="flex items-center gap-4 text-sm">
                      <Badge variant="outline">
                        Б: {recipe.total_protein ? Math.round(recipe.total_protein) : 0}г
                      </Badge>
                      <Badge variant="outline">
                        Ж: {recipe.total_fat ? Math.round(recipe.total_fat) : 0}г
                      </Badge>
                      <Badge variant="outline">
                        У: {recipe.total_carbs ? Math.round(recipe.total_carbs) : 0}г
                      </Badge>
                    </div>
                  )}
                </div>
