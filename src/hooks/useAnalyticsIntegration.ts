...
export const useInteractionTracking = () => {
  ...
  const trackSearchQuery = (query: string, results?: number, properties?: Record<string, any>) => {
...
export const useBusinessTracking = () => {
  ...
  const trackBookingEvent = (action: 'created' | 'cancelled' | 'updated', properties?: Record<string, any>) => {
    trackBooking(action, properties);
  };

  const trackOrderEvent = (action: 'created' | 'completed' | 'cancelled', properties?: Record<string, any>) => {
    trackOrder(action, properties);
  };

  const trackConversionEvent = (conversionType: string, value?: number, properties?: Record<string, any>) => {
