...
export const useAIRecommendations = () => {
  ...
  const loadUserBehavior = useCallback(async () => {
    ...
    try {
      ...
      const mockBehavior: UserBehavior = {
        visitHistory: [
          { vendor: 'Dolce Italia', dish: 'Margherita Pizza', rating: 5, timestamp: '2024-01-20T12:00:00Z' },
          { vendor: 'Spicy Asia', dish: 'Pad Thai', rating: 4, timestamp: '2024-01-18T18:30:00Z' },
          { vendor: 'Wild Bali', dish: 'Nasi Goreng', rating: 5, timestamp: '2024-01-15T19:00:00Z' }
        ],
  ...
  const generateInsights = useCallback((prefs: UserPreferences, behavior: UserBehavior): AIInsights => {
    // Analyze user profile
    const foodPersonality = behavior.tasteCompassProgress.favoriteSectors.includes('spice') ? 'Adventurous' : 'Traditional';
    const riskTaker = behavior.tasteCompassProgress.completedSectors.length > 5;
    const socialDiner = behavior.bookingHistory.length > 0;
    const healthConscious = prefs.healthGoals.length > 0;
    const adventurous = prefs.spiceLevel === 'hot' || prefs.spiceLevel === 'extreme';
    ...
    const trendingCuisines = behavior.visitHistory
      .reduce((acc, visit) => {
    ...
    // Find opportunities
    const allSectors = ['spice', 'smoke', 'ferment', 'umami', 'sweet', 'herb', 'salt', 'bitter'];
    const unexploredSectors = allSectors.filter(sector => 
  ...
  const generateRecommendations = useCallback((
    ...
    if (prefs.favoriteCuisines.includes('italian')) {
      recommendations.push({
        ...
        confidence: 0.9,
        reason: 'Based on your love for Italian cuisine and high ratings for pasta dishes',
    ...
    if (behavior.visitHistory.some(v => v.vendor === 'Spicy Asia' && v.rating >= 4)) {
      recommendations.push({
        id: 'rec-2',
        type: 'vendor',
        title: 'Wild Bali',
        ...
        confidence: 0.8,
        reason: 'Similar to your favorite Spicy Asia, but with Indonesian flavors',
    ...
    if (behavior.bookingHistory.some(b => b.event.includes('Wine') && b.satisfaction >= 4)) {
      recommendations.push({
        id: 'rec-3',
        type: 'event',
        ...
        confidence: 0.85,
        reason: 'You enjoyed our wine tasting event, this is a natural next step',
  ...
  const loadRecommendations = useCallback(async () => {
    ...
    try {
      ...
      const generatedRecommendations = generateRecommendations(preferences, behavior, generatedInsights);
  ...
  const getRecommendationsByConfidence = useCallback((minConfidence: number) => {
    return recommendations.filter(rec => rec.confidence >= minConfidence);
  }, [recommendations]);

  // Rate a recommendation
  const rateRecommendation = useCallback(async (recommendationId: string, rating: number) => {
    // In a real app, this would send feedback to the AI system
    console.log(`Rated recommendation ${recommendationId} with ${rating} stars`);
