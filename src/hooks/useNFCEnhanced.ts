...
export const useNFCEnhanced = () => {
  ...
  const loadUserProgress = useCallback(async () => {
    ...
    try {
      ...
      const mockProgress = {
        ...
        level: 2,
        achievements: [
          { id: '1', name: 'First Visit', description: 'Посетили первый сектор', unlocked: true },
          { id: '2', name: 'Spice Master', description: 'Завершили Spicy Asia', unlocked: true }
        ]
      };
  ...
  const updateUserProgress = useCallback(async (sectorName: string, sectorData: any) => {
    if (!user) return;
    ...
    try {
      ...
      setUserProgress(prev => {
        ...
        newProgress.completedSectors = Math.min(newProgress.completedSectors + 1, newProgress.totalSectors);
  ...
  // Проверка достижений
  const checkAchievements = useCallback(async (sectorName: string) => {
    if (!user) return;
  ...
  const writeNFC = useCallback(async (data: any) => {
    if (!nfcStatus.isSupported) {
      toast({
        title: "NFC не поддерживается",
