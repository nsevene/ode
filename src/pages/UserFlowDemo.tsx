...
const UserFlowDemo: React.FC = () => {
  ...
  const mockMenuItems = [
    { id: '1', name: 'Паста Карбонара', price: 45000, category: 'Итальянская кухня' },
    { id: '2', name: 'Суши Ролл', price: 35000, category: 'Японская кухня' },
    { id: '3', name: 'Бургер Классик', price: 55000, category: 'Американская кухня' },
    { id: '4', name: 'Салат Цезарь', price: 25000, category: 'Здоровое питание' }
  ];
  ...
  const handleStartOrdering = () => {
    ...
    addNotification({
      type: 'info',
      title: 'Начало заказа',
      message: 'Вы перешли в раздел меню. Добавьте блюда в корзину для оформления заказа.'
    });
  };
  ...
  const handleStartBooking = () => {
    ...
    addNotification({
      ...
      message: 'Вы перешли в раздел бронирования. Выберите дату и время для столика.'
    });
  };
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Выберите сценарий для демонстрации навигации и функциональности приложения
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-2 border-blue-200">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-gray-600">
              Полный цикл заказа: просмотр меню → добавление в корзину → оформление → оплата
            </p>
            ...
            {cart.length > 0 && (
              <div className="bg-blue-50 p-3 rounded-lg">
                <p className="text-sm font-medium text-blue-800">
                  В корзине: {getCartItemsCount()} товаров на {getCartTotal().toLocaleString()} ₽
                </p>
              </div>
            )}
            ...
            <div className="space-y-2">
              {cart.length === 0 ? (
                ...
                <div className="space-y-2">
                  ...
                  <Button onClick={handleCompleteOrder} variant="outline" className="w-full">
                    Завершить заказ
                  </Button>
                </div>
              )}
        ...
        <Card className="border-2 border-green-200">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-gray-600">
              Бронирование столика: выбор даты → выбор времени → заполнение данных → подтверждение
            </p>
            ...
            <div className="space-y-2">
              {!currentBooking ? (
                ...
                <div className="space-y-2">
                  ...
                  <Button onClick={handleCompleteBooking} variant="outline" className="w-full">
              ...
              {currentBooking ? (
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm">{currentBooking.date || 'Не выбрана'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">Время:</span>
                    <span className="text-sm">{currentBooking.time || 'Не выбрано'}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <Badge variant={currentBooking.status === 'confirmed' ? 'default' : 'secondary'}>
                      {currentBooking.status === 'confirmed' ? 'Подтверждено' : 'Ожидает'}
                    </Badge>
                  </div>
                </div>
              ) : (
                <p className="text-gray-500 text-sm">Нет активного бронирования</p>
              )}
            </CardContent>
          </Card>
        </div>
