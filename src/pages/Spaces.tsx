...
const Spaces = () => {
  ...
  const getMockSpaces = (): Space[] => [
    {
      ...
      description: 'Вся выпечка + классические десерты в комфортной зоне с кондиционером',
      area: '100 м²',
      floor: 1,
      capacity: 40,
      price_per_hour: 5000,
    ...
    },
    {
      ...
      amenities: ['Кухонное оборудование', 'WiFi', 'Звуковая система', 'Освещение'],
      availability: {
        monday: ['10:00-22:00'],
        tuesday: ['10:00-22:00'],
      ...
      equipment_included: ['Кухонное оборудование', 'Столы', 'Стулья', 'Освещение'],
    ...
    },
    {
      ...
      amenities: ['Открытая терраса', 'WiFi', 'Звуковая система', 'Освещение', 'Кухня'],
      availability: {
        monday: ['10:00-22:00'],
        tuesday: ['10:00-22:00'],
        wednesday: ['10:00-22:00'],
  ...
  useEffect(() => {
    ...
    if (selectedFloor !== 'all') {
      filtered = filtered.filter(space => space.floor.toString() === selectedFloor);
    }
  ...
  }, [spaces, searchQuery, selectedFloor, selectedCategory, selectedCapacity, sortBy]);
        ...
        <div className="container mx-auto px-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-muted-foreground">({space.review_count})</span>
                        </div>
                        <div className="text-right">
                          <div className="font-semibold">{space.price_per_hour.toLocaleString()} ₽/час</div>
                          <div className="text-xs text-muted-foreground">{space.price_per_day.toLocaleString()} ₽/день</div>
                        </div>
                      </div>
          ...
          {filteredSpaces.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold mb-2">Пространства не найдены</h3>
          ...
          {selectedSpace && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">{selectedSpace.location}</p>
                <p className="text-sm text-muted-foreground">{selectedSpace.area} • {selectedSpace.capacity} мест</p>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setGuests(Math.min(selectedSpace.max_guests, guests + 1))}
                    >
                      <Plus className="h-4 w-4" />
                    </Button>
                  </div>
                  <p className="text-xs text-muted-foreground">Максимум: {selectedSpace.max_guests} гостей</p>
                </div>
                
                <div className="space-y-2">
                  <label className="text-sm font-medium">Продолжительность (часы)</label>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setDuration(Math.max(selectedSpace.min_booking_hours, duration - 1))}
                    >
                      <Minus className="h-4 w-4" />
                    </Button>
                  ...
                  <p className="text-xs text-muted-foreground">Минимум: {selectedSpace.min_booking_hours} часов</p>
                </div>
              </div>
              ...
              <div className="bg-muted/50 p-4 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-bold text-lg">{selectedSpace.price_per_hour * duration} ₽</span>
                </div>
