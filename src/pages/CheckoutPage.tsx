...
import { sendOrderConfirmation, type OrderConfirmationEmail } from '@/lib/email';
...
const CheckoutPage: React.FC = () => {
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      // Здесь будет создание заказа в базе данных
      // Пока что симулируем успешный заказ
      ...
      if (user?.email) {
        const orderData: OrderConfirmationEmail = {
          ...
          estimatedDelivery: new Date(Date.now() + 60 * 60 * 1000).toLocaleString('ru-RU')
        };
                ...
                {paymentMethod === 'card' && (
                  <div className="mt-6">
                    <StripePaymentForm
                      ...
                      description={`Заказ в ODE Food Hall на ${totalPrice.toFixed(0)} ₽`}
                    />
                  </div>
                )}
