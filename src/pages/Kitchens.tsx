...
import { 
  ChefHat, Coffee, Utensils, Leaf, FlameKindling, Wheat, Cookie, Fish, 
  Search, Filter, Star, Clock, MapPin, Users, Phone, Calendar, Heart,
  BookOpen, Award, TrendingUp, Eye, Share2, MessageCircle, ThumbsUp
} from 'lucide-react';
...
const Kitchens = () => {
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== 'all') {
      filtered = filtered.filter(kitchen => kitchen.category === selectedCategory);
    }

    // Cuisine filter
    if (selectedCuisine !== 'all') {
      filtered = filtered.filter(kitchen => kitchen.cuisine_type === selectedCuisine);
    }
  ...
  const getMockKitchens = (): Kitchen[] => [
    {
      ...
      wait_time: 15,
      popular_dishes: [
        { name: 'Margherita Pizza', price: 450, description: 'Classic Italian pizza with fresh tomatoes and mozzarella' },
        { name: 'Spaghetti Carbonara', price: 380, description: 'Traditional Roman pasta with eggs and pancetta' },
        { name: 'Tiramisu', price: 180, description: 'Classic Italian dessert with coffee and mascarpone' }
      ]
    },
    {
      id: 'spicy-asia',
      name: 'Spicy Asia',
      description: 'Authentic Asian street food with bold flavors and fresh ingredients',
      category: 'asian',
      cuisine_type: 'Asian',
      area: 20,
      ...
      wait_time: 20,
      popular_dishes: [
        { name: 'Pad Thai', price: 380, description: 'Traditional Thai stir-fried noodles' },
        { name: 'Tonkotsu Ramen', price: 420, description: 'Rich pork bone broth ramen' },
        { name: 'Xiao Long Bao', price: 280, description: 'Shanghai soup dumplings' }
      ]
    },
    {
      id: 'wild-bali',
      name: 'Wild Bali',
      description: 'Traditional Balinese cuisine with modern presentation and authentic flavors',
      category: 'indonesian',
      ...
      wait_time: 12,
      popular_dishes: [
        { name: 'Nasi Goreng', price: 320, description: 'Indonesian fried rice with chicken and vegetables' },
        { name: 'Babi Guling', price: 450, description: 'Traditional Balinese roasted pork' },
        { name: 'Gado-gado', price: 280, description: 'Indonesian vegetable salad with peanut sauce' }
      ]
    },
    {
      id: 'green-garden',
      name: 'Green Garden',
      description: 'Fresh vegetarian and vegan cuisine with organic ingredients',
      category: 'vegetarian',
      ...
      wait_time: 8,
      popular_dishes: [
        { name: 'Acai Bowl', price: 280, description: 'Fresh acai bowl with granola and fruits' },
        { name: 'Quinoa Salad', price: 320, description: 'Healthy quinoa salad with vegetables' },
        { name: 'Green Smoothie', price: 180, description: 'Fresh green smoothie with spinach and fruits' }
      ]
    },
    {
      id: 'sweet-dreams',
      ...
      wait_time: 5,
      popular_dishes: [
        { name: 'Chocolate Lava Cake', price: 180, description: 'Warm chocolate cake with molten center' },
        { name: 'Macarons (6pcs)', price: 120, description: 'French macarons in various flavors' },
  ...
  const cuisines = [...new Set(kitchens.map(kitchen => kitchen.cuisine_type))];
  ...
  const handleShare = (kitchen: Kitchen) => {
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(window.location.origin + `/kitchen/${kitchen.slug}`);
      toast({
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
              От балийского огня до неаполитанского жара — исследуйте каждый вкус под одной крышей.
            </p>
          </div>
          ...
          <div className={viewMode === 'grid' 
            ...
            {filteredKitchens.map((kitchen) => {
              ...
              return (
                <Card key={kitchen.id} className="hover:shadow-lg transition-shadow duration-300">
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mb-2">{kitchen.description}</p>
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {kitchen.is_halal && <Badge variant="secondary" className="text-xs">Halal</Badge>}
                          {kitchen.is_vegetarian_friendly && <Badge variant="secondary" className="text-xs">Veg</Badge>}
                          {kitchen.is_vegan_friendly && <Badge variant="secondary" className="text-xs">Vegan</Badge>}
                          {kitchen.is_spicy && <Badge variant="destructive" className="text-xs">Spicy</Badge>}
                        </div>
                      </div>
                      <Button
                        variant="ghost"
                  ...
                  <CardContent className="pt-0">
                    <div className="space-y-3">
                      ...
                      <div className="text-sm">
                        <p className="font-medium">Шеф-повар: {kitchen.chef_name}</p>
                        <p className="text-muted-foreground">{kitchen.chef_experience} лет опыта</p>
                      </div>

                      {/* Specialties */}
                      <div className="flex flex-wrap gap-1">
                        {kitchen.specialties.slice(0, 3).map((specialty, index) => (
                      ...
                      {kitchen.awards.length > 0 && (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm text-muted-foreground">
                            {kitchen.awards[0]}
                            {kitchen.awards.length > 1 && ` +${kitchen.awards.length - 1} наград`}
                          </span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm text-muted-foreground">{kitchen.location}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          {kitchen.is_open ? (
          ...
          {selectedKitchen && (
            <div className="space-y-4">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">{selectedKitchen.location}</p>
              </div>
                ...
                <Select>
                  ...
                  <SelectContent>
                    {[1, 2, 3, 4, 5, 6, 7, 8].map(num => (
                      <SelectItem key={num} value={num.toString()}>{num} {num === 1 ? 'гость' : 'гостей'}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
