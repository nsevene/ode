...
function SectionTitle({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <h2 className={`mb-6 text-2xl font-serif text-brand-forest md:text-3xl ${className}`}>{children}</h2>
  );
}
...
const benefits = [
  { id: 1, icon: Truck, title: "Daily 500 visitors", desc: "Stable foot traffic" },
  { id: 2, icon: Users, title: "Marketing Kit", desc: "QR, photos, promos" },
  { id: 3, icon: Check, title: "Easy 10 %", desc: "Auto-calculated commission" },
];
...
const BreakfastCard = ({ title, price, badge }: any) => (
  <div className="rounded-xl border bg-white p-4 shadow-sm">
    ...
    {badge && <span className="mb-2 inline-block rounded bg-brand-golden/20 px-2 text-xs text-brand-golden">{badge}</span>}
...
const PlanCard = ({ title, days, price, popular }: any) => (
  <div className={`rounded-xl border bg-white p-6 shadow-sm ${popular ? "border-brand-golden scale-105" : ""}`}>
    {popular && <span className="mb-2 inline-block rounded bg-brand-golden px-3 py-0.5 text-xs font-medium text-brand-forest">Most Popular</span>}
    ...
    <Button className="mt-6 w-full bg-brand-golden/90 hover:bg-brand-golden">Add to Cart</Button>
  </div>
);
...
const BasketCard = ({ title, price }: any) => (
  <div className="rounded-xl border bg-white p-6 shadow-sm">
    ...
    <Button className="w-full bg-brand-forest hover:bg-brand-forest/90">Add to Cart</Button>
  </div>
);
...
function RevenueCalculator() {
  ...
  return (
    <div className="mx-auto mt-6 max-w-lg rounded-xl border bg-white p-6 shadow-sm">
      <p className="mb-4 text-sm text-brand-forest/70">Interactive calculator coming soon …</p>
    </div>
  );
}
...
function PartnerCTA() {
  return (
    <div className="mt-10 flex flex-col items-center gap-4 md:flex-row md:justify-center">
      <Button size="lg" className="bg-brand-golden/90 hover:bg-brand-golden">WhatsApp (fast)</Button>
      <Button variant="outline" size="lg">Download PDF</Button>
      <Button variant="secondary" size="lg">Schedule Call</Button>
    </div>
  );
}