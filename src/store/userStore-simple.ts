  ...
  devtools(
    persist(
      (set, get) => ({
            ...
            if (response.success && response.data) {
              set({ 
                ...
                pagination: {
                  ...
                  totalPages: Math.ceil(response.data.length / get().pagination.limit)
                }
              });
            } else {
        ...
        promoteToAdmin: async (id: string) => {
          ...
          try {
            ...
            if (response.success) {
              set((state) => ({
                users: state.users.map(user => 
                  user.id === id ? { ...user, role: USER_ROLES.ADMIN } : user
                )
              }));
              return true;
            } else {
              set({ error: response.error || 'Failed to promote user to admin' });
              return false;
            }
          } catch (error) {
            set({ error: error instanceof Error ? error.message : 'Unknown error' });
        ...
        demoteFromAdmin: async (id: string) => {
          ...
          try {
            ...
            if (response.success) {
              set((state) => ({
                users: state.users.map(user => 
                  user.id === id ? { ...user, role: USER_ROLES.GUEST } : user
                )
              }));
              return true;
            } else {
              set({ error: response.error || 'Failed to demote user from admin' });
              return false;
            }
          } catch (error) {
            set({ error: error instanceof Error ? error.message : 'Unknown error' });
        ...
        clearUsers: () => {
          set({ users: [], currentUser: null, selectedUser: null });
        }
      })),
      {
        name: 'user-store',
