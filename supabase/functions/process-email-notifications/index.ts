    ...
    for (const notification of notifications) {
      if (notification.data?.trigger_email) {
        ...
        if (emailType === 'referral_bonus' || emailType === 'welcome_bonus') {
          ...
          const { error: emailError } = await supabaseClient.functions.invoke('send-referral-emails', {
            body: {
              type: emailType,
          ...
          if (emailError) {
            ...
            console.log(`Successfully sent ${emailType} email to user ${notification.user_id}`)
