...
export const useNFC = () => {
  ...
  const handleNFCRead = (event: any) => {
    ...
    const nfcData = sectorData || {
      ...
      sectors_visited: ['ferment', 'smoke', 'spice'].slice(
        0,
  ...
  const startReading = async () => {
    if (!nfcSupported) {
      const message =
        'NFC не поддерживается на этом устройстве. Используйте QR-код для навигации.';
      setError(message);
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        setTimeout(() => {
          const demoData = {
            ...
            sectors_visited: ['ferment', 'smoke', 'spice', 'sweet'].slice(
              0,
          ...
          window.dispatchEvent(
            new CustomEvent('nfcRead', { detail: demoData })
          );
        ...
        if ('NDEFReader' in window) {
          ...
          setTimeout(() => {
            const demoData = {
              ...
              sectors_visited: ['spice', 'ferment'].slice(
                0,
