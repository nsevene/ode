    ...
    if (userProgress && notificationsEnabled) {
      ...
      if (newAchievements && newAchievements.length > 0) {
        newAchievements.forEach((achievement: any) => {
          sendNotification('🏆 Новое достижение!', achievement.description);
        });
      }
    }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="text-center">
          <CardContent className="pt-4">
            <Wifi
              className={`h-6 w-6 mx-auto mb-2 ${isOnline ? 'text-green-500' : 'text-red-500'}`}
            />
        ...
        <Card className="text-center">
          <CardContent className="pt-4">
            <Nfc
              className={`h-6 w-6 mx-auto mb-2 ${nfcStatus.isSupported ? 'text-green-500' : 'text-red-500'}`}
            />
        ...
        <Card className="text-center">
          <CardContent className="pt-4">
            <Bell
              className={`h-6 w-6 mx-auto mb-2 ${notificationsEnabled ? 'text-green-500' : 'text-gray-400'}`}
            />
        ...
        <Card className="text-center">
          <CardContent className="pt-4">
            <Camera
              className={`h-6 w-6 mx-auto mb-2 ${cameraPermission ? 'text-green-500' : 'text-gray-400'}`}
            />
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>Прогресс</span>
                <span>
                  {userProgress.completedSectors}/{userProgress.totalSectors}
                </span>
              </div>
              <Progress
                value={
