...
export const ABTestingDashboard: React.FC = () => {
      ...
      const mockTests: ABTest[] = [
        {
          ...
          description:
            'Тестирование эффективности разных кнопок призыва к действию',
          status: 'running',
        ...
        },
        {
          ...
          description:
            'Сравнение отображения цен: полная стоимость vs. стоимость за человека',
          status: 'completed',
          test_type: 'pricing',
          traffic_split: 50,
          ...
          variant_b: {
            name: 'За человека',
            content: {
              display_type: 'per_person',
              format: 'от IDR 125,000/чел',
            },
            conversions: 41,
            views: 667,
          },
  ...
  const getConfidenceLevel = (test: ABTest): number => {
    ...
    const aRate = getConversionRate(
      test.variant_a.conversions,
      test.variant_a.views
    );
    const bRate = getConversionRate(
      test.variant_b.conversions,
      test.variant_b.views
    );
  ...
  const toggleTestStatus = async (testId: string) => {
    ...
    setTests(
      tests.map((t) => (t.id === testId ? { ...t, status: newStatus } : t))
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Оптимизация конверсий через тестирование
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {tests
                .reduce(
                  (sum, t) => sum + t.variant_a.views + t.variant_b.views,
                  0
                )
                .toLocaleString()}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {tests.reduce(
                (sum, t) =>
                  sum + t.variant_a.conversions + t.variant_b.conversions,
                0
              )}
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {test.variant_a.conversions} из {test.variant_a.views}{' '}
                      просмотров
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {test.variant_b.conversions} из {test.variant_b.views}{' '}
                      просмотров
                    </div>
