  ...
  const validatePromoCode = async (code: string) => {
    ...
    try {
      ...
      if (
        ...
        new Date(promoData.valid_until) < new Date()
      ) {
        setValidationResult({
          valid: false,
      ...
      if (
        ...
        promoData.usage_count >= promoData.usage_limit
      ) {
        setValidationResult({
          valid: false,
      ...
      if (
        ...
        orderAmount < promoData.min_order_amount
      ) {
        setValidationResult({
          valid: false,
    ...
    try {
      ...
      if (validationResult.discount_type === 'percentage') {
        discountAmount = Math.round(
          orderAmount * (validationResult.discount_value! / 100)
        );
        ...
        {appliedCode ? (
          <Alert>
            ...
            <AlertDescription className="flex items-center justify-between">
              <span>
                Применен код: <strong>{appliedCode}</strong>
              </span>
