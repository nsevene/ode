  ...
  const getCurrentLevelProgress = () => {
    ...
    const currentLevelMinXP =
      progress.user_level === 1
        ? 0
        : baseXP * Math.pow(1.5, progress.user_level - 1);
    const nextLevelXP = baseXP * Math.pow(1.5, progress.user_level);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card
          className={`bg-gradient-to-br border-opacity-20 ${getTierColor(loyaltyData?.tier || 'bronze')}`}
        >
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
              ...
              <Badge variant="secondary">{loyaltyData?.tier || 'Bronze'}</Badge>
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              return (
                ...
                >
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <Target
                        className={`w-5 h-5 ${isCompleted ? 'text-green-500' : 'text-blue-500'}`}
                      />
        ...
        <TabsContent value="rewards" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {[
              ...
              <Card
                key={reward.id}
                className={`${!reward.available ? 'opacity-60' : ''}`}
              >
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <Gift className="w-6 h-6 text-purple-500" />
                    <Badge variant="secondary">{reward.points} очков</Badge>
                  </div>
                </CardHeader>
                <CardContent>
