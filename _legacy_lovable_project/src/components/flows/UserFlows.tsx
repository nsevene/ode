...
export const UserFlows: React.FC<UserFlowProps> = ({ children }) => {
  ...
  const { cart, currentBooking, getCartItemsCount, addNotification } =
    useAppContext();
        ...
        return [
          {
            id: 'menu',
            label: 'Меню',
            completed: location.pathname.startsWith('/menu'),
          },
          { id: 'cart', label: 'Корзина', completed: cart.length > 0 },
          {
            id: 'checkout',
            label: 'Оформление',
            completed: location.pathname.startsWith('/checkout'),
          },
          {
            id: 'payment',
            label: 'Оплата',
            completed: location.pathname.startsWith('/payment'),
          },
        ];
      case 'booking':
        return [
          ...
          { id: 'details', label: 'Детали', completed: !!currentBooking?.name },
          {
            id: 'confirmation',
            label: 'Подтверждение',
            completed: currentBooking?.status === 'confirmed',
          },
        ];
      case 'auth':
        return [
          {
            id: 'login',
            label: 'Вход',
            completed: location.pathname.includes('login'),
          },
          {
            id: 'register',
            label: 'Регистрация',
            completed: location.pathname.includes('register'),
          },
          {
            id: 'profile',
            label: 'Профиль',
            completed: location.pathname.includes('profile'),
          },
        ];
      default:
        return [];
