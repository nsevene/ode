...
const ChunkRecovery = () => {
  ...
  useEffect(() => {
    const alreadyRecovered =
      sessionStorage.getItem('chunk-recovery-done') === '1';
    ...
    const onError = (event: ErrorEvent) => {
      if (
        shouldHandle(
          event.message,
          (event.error && (event.error as any).name) || undefined
        )
      ) {
    ...
    const onRejection = (event: PromiseRejectionEvent) => {
      ...
      if (shouldHandle(reason, name)) {
        try {
          // eslint-disable-next-line no-console
          console.error('ChunkRecovery onRejection:', {
            reason,
            name,
            reasonObj: event.reason,
          });
        } catch {}
        event.preventDefault?.();
        clearCachesAndReload();
