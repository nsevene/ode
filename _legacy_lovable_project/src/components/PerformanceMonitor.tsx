...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  const updateMetrics = useCallback(() => {
    ...
    const renderTime = Math.max(
      0,
      Math.round(performance.now() - (performance.timeOrigin || 0))
    );
  ...
  const calculateCacheHitRate = useCallback(() => {
    ...
    if (cache) {
      const hitRate = Math.round(
        (cache.transferSize / cache.encodedBodySize) * 100
      );
      ...
      >
          ...
          {!isMinimized && (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <span>Cache:</span>
                <span
                  className={getStatusColor(100 - metrics.cacheHitRate, {
                    good: 20,
                    warning: 50,
                  })}
                >
              ...
              <div className="flex items-center justify-between">
                <span>Bundle:</span>
                <span
                  className={getStatusColor(metrics.bundleSize, {
                    good: 500,
                    warning: 1000,
                  })}
                >
        ...
        {isVisible && (
          ...
          >
            ...
            {metrics.fps < 30 && (
              <div className="text-red-400 text-xs">⚠️ Low FPS detected</div>
            )}
            {metrics.loadTime > 3000 && (
              <div className="text-yellow-400 text-xs">⚠️ Slow page load</div>
            )}
            {metrics.networkStatus === 'offline' && (
              <div className="text-red-400 text-xs">⚠️ Offline mode</div>
            )}
          </motion.div>
        )}
      </motion.div>
...
export const PerformanceSuggestions: React.FC = () => {
  ...
  if (metrics.memoryUsage > 80) {
    suggestions.push({
      type: 'warning',
      message:
        'High memory usage detected. Consider reducing component complexity or implementing lazy loading.',
  ...
  if (metrics.fps < 30) {
    suggestions.push({
      type: 'error',
      message:
        'Low FPS detected. Consider reducing animations or optimizing rendering.',
  ...
  if (metrics.loadTime > 3000) {
    suggestions.push({
      type: 'warning',
      message:
        'Slow page load detected. Consider optimizing bundle size or implementing code splitting.',
  ...
  if (suggestions.length === 0) {
    return (
      <div className="text-green-500 text-sm">✅ Performance is optimal</div>
    );
  }
