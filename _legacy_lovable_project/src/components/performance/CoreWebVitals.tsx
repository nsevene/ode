...
export const CoreWebVitals = () => {
  ...
  const metrics = [
    ...
    },
    {
      ...
      icon: Activity,
      description: 'Стабильность визуального макета',
    },
    {
      key: 'FCP',
      name: 'First Contentful Paint',
      icon: Clock,
      description: 'Время появления первого контента',
    },
    {
      key: 'TTFB',
          ...
          return (
            <div key={key} className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Badge variant={status}>{formatValue(key, value)}</Badge>
              </div>
              <div className="text-xs text-muted-foreground">{description}</div>
              <Progress
                value={Math.min(
                  (value / (key === 'CLS' ? 0.5 : 5000)) * 100,
                  100
                )}
                className="h-2"
              />
