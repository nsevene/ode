...
const ImprovedNavigation = () => {
  ...
  const navigationItems: NavigationItem[] = [
    ...
    },
    {
      ...
      icon: Database,
      category: 'admin',
      description: 'Управление контентом и сущностями',
    },
    {
      href: '/games-admin',
      label: 'Games',
  ...
  return (
    <>
      ...
      <nav
        ...
        className="bg-pure-white/95 backdrop-blur-lg shadow-soft sticky top-0 z-40 border-b border-cream-medium/50 transition-all duration-300 hover:shadow-medium"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            ...
            <div className="flex items-center">
              <Link
                to="/"
                className="flex-shrink-0 flex items-center hover:scale-105 transition-all duration-300 group"
              >
                <div className="flex flex-col justify-center">
                  <div className="text-xl font-bold text-charcoal group-hover:text-forest-green transition-colors leading-tight">
                    ODE
                  </div>
                </div>
              </Link>
            ...
            <div className="hidden lg:flex items-center space-x-8">
              {getItemsByCategory('primary').map((item) =>
                item.isAnchor ? (
                  ...
                  >
                    <item.icon
                      className={`w-4 h-4 transition-transform group-hover:scale-110 ${isActive(item.href) ? 'text-forest-green' : ''}`}
                    />
                    <span className="story-link">{item.label}</span>
                    {item.badge && (
                      <Badge variant="secondary" className="text-xs ml-1">
            ...
            <div className="hidden lg:flex items-center space-x-4">
              <LanguageSwitcher
                currentLanguage={
                  i18n.language as 'en' | 'zh' | 'es' | 'de' | 'id' | 'ms'
                }
            ...
            <div className="px-4 py-6 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-1">
                  {[
                    ...getItemsByCategory('primary'),
                    ...getItemsByCategory('secondary'),
                  ].map((item) => (
                    <Link
                      key={item.href}
                      to={item.href}
