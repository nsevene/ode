...
const ResponsiveTester: React.FC<ResponsiveTestProps> = ({
  className = '',
}) => {
  const [testResults, setTestResults] = useState<Record<string, boolean>>({});
  const [isTestingViewports, setIsTestingViewports] = useState(false);
  const [currentViewport, setCurrentViewport] = useState<ViewportTest | null>(
    null
  );
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {Object.entries(breakpointResults).map(([name, matches]) => (
              <div
                key={name}
                className={`p-4 rounded-lg border ${getStatusColor(matches)}`}
              >
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            <div className="text-sm text-blue-800">
              <strong>Текущий размер:</strong> {window.innerWidth} ×{' '}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-red-50 rounded-lg">
                <div className="text-2xl font-bold text-red-600">
                  {
                    Object.values(testResults).filter((result) => !result)
                      .length
                  }
                </div>
